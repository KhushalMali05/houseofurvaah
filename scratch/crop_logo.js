...
function cropPNG(filePath) {
  ...
  console.log(`Original PNG info: ${width}x${height}, BitDepth: ${bitDepth}, ColorType: ${colorType}`);

  if (colorType !== 6 || bitDepth !== 8) {
    console.log('Not RGBA 8-bit, adjusting CSS scale instead.');
    return false;
  }

  const decompressed = zlib.inflateSync(Buffer.concat(idatBuffers));
  const rowBytes = width * 4 + 1; // 1 byte filter per scanline
  ...
  // Find non-transparent bounding box (alpha > 5)
  ...
  console.log(`Cropped dimensions: ${cropW}x${cropH} (Saved space: ${width}x${height} -> ${cropW}x${cropH})`);
